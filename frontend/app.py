...
# Header
st.markdown('<p class="main-header">🛍️ Amazon Review Sentiment Analyzer</p>', unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #666; font-size: 1.2rem;'>Analyze customer reviews with AI-powered sentiment scoring</p>", unsafe_allow_html=True)

# Sidebar
with st.sidebar:
    st.markdown("### 📋 Navigation")
    page = st.radio("", ["📤 Upload & Process", "📊 View Analytics", "ℹ️ Help"])
    ...
    try:
        ...
        st.metric("Total Reviews in DB", f"{count:,}")
        conn.close()
    except:
        st.info("No data yet")
...
if page == "📤 Upload & Process":
    ...
    if uploaded_file is not None:
        ...
        st.markdown("---")
        st.markdown("## ⚙️ Step 2: Configure Columns")
        ...
        with col1:
            ...
            if len(sample_text) > 0:
                ...
                st.error("⚠️ This column appears to be empty!")
        ...
        with col_btn2:
            if st.button("🎯 Analyze Reviews & Store Results", type="primary", use_container_width=True):
                ...
                if len(str(df[review_column].iloc[0])) == 0:
                    ...
                    with st.spinner('🔄 Processing reviews... This may take a moment...'):
                        ...
                        for idx, row in df.iterrows():
                            ...
                            if idx % 50 == 0:
                                ...
                                status_text.text(f"Processing: {idx + 1}/{total_rows} reviews ({progress*100:.1f}%)")
                            
                            # Debug first 3
                            if show_debug and idx < 3:
                                with debug_container:
                                    st.write(f"**Row {idx+1}:** Score={score}, Category={category}, Text={review[:100]}...")
                        ...
                        with col_m4:
                            ...
                            st.metric("👍 Positive %", f"{positive/len(scores)*100:.1f}%")
                        
                        st.info("💡 Go to 'View Analytics' to see detailed insights!")
...
# ==================== PAGE 2: VIEW ANALYTICS ====================
    ...
    try:
        ...
        if len(df_db) == 0:
            st.warning("⚠️ No data found. Please upload and process data first!")
        else:
            # Overview metrics
            ...
            with col_bottom:
                st.markdown("### ⚠️ Bottom 5 Worst Reviews")
